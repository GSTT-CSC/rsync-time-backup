...
# -----------------------------------------------------------------------------
fn_display_usage() {
	echo "Usage: $(basename $0) [OPTION]... <[USER@HOST:]SOURCE> <[USER@HOST:]DESTINATION> <[USER@HOST:]SYMLINK DESTINATION> [exclude-pattern-file]"
	echo ""
	echo "Options"
...
while : ; do
	...
	fn_rm_file "$SYMLINK_FOLDER/latest"
#	fn_ln "$(basename -- "$DEST")" "$SYMLINK_FOLDER/latest"
	fn_ln "$DEST" "$SYMLINK_FOLDER/latest"
